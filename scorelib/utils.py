    ...
    for entry in valid_data_list:
        ...
        if part_name and page_string:
            ...
            if len(writer.pages) > 0:
                ...
                filename = f"{safe_title}_{safe_part}-id{piece.id}.pdf".replace(
                    " ", "_"
                )

                # Save file
                new_part.pdf_file.save(
...
def process_audio_file_logic(recording_obj):
    ...
    """
    if not recording_obj.audio_file or not os.path.exists(
        recording_obj.audio_file.path
    ):
        return
    ...
    safe_piece = "".join(
        x for x in recording_obj.piece.title if x.isalnum() or x in "._- "
    )
    safe_concert = "".join(
        x for x in recording_obj.concert.title if x.isalnum() or x in "._- "
    )
    # include description but only keep safe characters and limit length to avoid filesystem issues
    safe_desc = "".join(
        x for x in recording_obj.description if x.isalnum() or x in "._- "
    )[:20]

    base_name = f"{safe_concert}_{safe_piece}_{safe_desc}_c{recording_obj.concert.id}_r{recording_obj.id}".replace(
        " ", "_"
    )
    ...
    if site_settings and site_settings.audio_ripping_enabled and shutil.which("ffmpeg"):
        ...
        cmd = [
            ...
            "6",  # good compression
            "-metadata",
            f"title={recording_obj.piece.title}",
            "-metadata",
            f"artist={site_settings.band_name}",
            "-metadata",
            f"album={recording_obj.concert.title}",
            "-metadata",
            f'date={recording_obj.concert.date.year if recording_obj.concert.date else ""}',
            "-metadata",
            f"comment={recording_obj.description}",  # description in comment tag
            new_full_path,
            "-y",
        ]
...
def get_orphaned_files():
    ...
    # Format: (Model, Feldname, [Unterordner], Typ)
    checks = [
        # Keep legacy folder "parts" for backwards compatibility with old uploads
        (Part, "pdf_file", ["sheet_music/parts", "parts"], "PDF (Noten)"),
        (AudioRecording, "audio_file", ["concerts/audio"], "Audio (Konzert)"),
    ]

    for model, field, subfolders, file_type in checks:
        # Datenbank-Bestand als Set für schnellen Abgleich
        files_in_db = {
            normalize_rel_path(path)
            for path in model.objects.exclude(**{field: ""})
        ...
        for subfolder in subfolders:
            ...
            for root, dirs, files in os.walk(full_dir_path):
                for filename in files:
                    # Erstelle den Pfad, wie er in der DB gespeichert wäre
                    rel_path = normalize_rel_path(
                        os.path.relpath(
                            os.path.join(root, filename), settings.MEDIA_ROOT
                        )
                    )

                    if rel_path in files_in_db or rel_path in seen_paths:
                        continue
