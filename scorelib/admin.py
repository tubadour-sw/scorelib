...
class ArrangerAdmin(admin.ModelAdmin):
    ...
    def merge_arrangers_action(self, request, queryset):
        ...
        return get_generic_merge_response(
            self, request, queryset, "Komponisten mergen", "merge_arrangers_action"
        )
        ...
        if request.method == "POST":
            ...
            if formset.is_valid() and master_pdf:  #
                ...
                if not valid_data_list:
                    self.message_user(
                        request,
                        "Bitte geben Sie bei mindestens einer Stimme sowohl den Namen als auch die Seiten an.",
                        messages.WARNING,
                    )
                else:
                    try:
...
class ConcertAdmin(admin.ModelAdmin):
    ...
    def rip_audio_link(self, obj):
        if not obj.pk:
            return "-"  # in case the concert is not saved yet
...
class GenreAdmin(admin.ModelAdmin):
    ...
    def merge_genres_action(self, request, queryset):
        ...
        return get_generic_merge_response(
            self, request, queryset, "Genres mergen", "merge_genres_action"
        )
...
class LoanRecordAdmin(admin.ModelAdmin):
    ...
    def get_type(self, obj):
        ...
        if obj.piece.is_owned_by_orchestra:
            return format_html(
                '<span style="color: #d63384;">↗ Verleih</span>'
            )  # Wir geben weg
        return format_html(
            '<span style="color: #0dcaf0;">↘ Fremd-Leihgabe</span>'
        )  # Wir holen her
